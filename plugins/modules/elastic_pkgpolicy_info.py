...
try:
    from ansible_collections.expedient.elastic.plugins.module_utils.kibana \
        import Kibana
except:
    import sys
    import os
...
def main():
    ...
    #     ('state', 'present', ('enabled', 'alert_type', 'conditions',
    #     'actions')), ('alert-type', 'metrics_threshold', ('conditions'))

    module = AnsibleModule(argument_spec=module_args, supports_check_mode=True,
                           mutually_exclusive=[('agent_policy_name',
                                                'agent_policy_id')],
                           required_one_of=[('agent_policy_name',
                                             'agent_policy_id')])
    ...
    try:
        ...
        results['agent_policy_status'] = "Agent Policy was not found. Cannot continue \
            without valid Agent Policy Name or ID"
        results['changed'] = False
    ...
    if integration_name:
        ...
        results['integration_status'] = "No Integration Name provided to get the \
            integration object"
        results['changed'] = False
        module.exit_json(**results)
    ...
    pkg_policy_object = kibana.get_pkg_policy(integration_name,
                                              agent_policy_id)
